<template>
  <div
    class="w-full h-full min-h-screen bg-zinc-950 text-white flex flex-col font-sans select-none overflow-hidden relative pb-[90px]">
    <div class="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
      <transition-group name="notification">
        <div v-for="notification in notifications" :key="notification.id"
          ...
          <div
            class="flex-shrink-0 w-10 h-10 bg-yellow-500/20 rounded-full flex items-center justify-center">
            <span class="text-xl">🏆</span></div>
          <div class="flex flex-col"><h3
            class="font-bold text-sm sm:text-base md:text-lg text-yellow-400">{{
              notification.title
            }}</h3>
            <p class="text-xs sm:text-sm text-zinc-300 leading-tight">{{
    ...
    <div class="pt-5 pb-2 px-4 z-20 shrink-0 flex flex-col gap-3 bg-zinc-950">
      <div class="flex justify-between items-center">
        <div v-if="currentAsset" class="flex items-center gap-3">
          <div
            class="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-xl bg-zinc-800 flex items-center justify-center border-2 border-zinc-700 shadow-xl">
            <span class="text-base sm:text-lg md:text-xl font-black text-zinc-300">{{
                currentAsset.symbol[0]
              }}</span>
          </div>
          <div class="flex flex-col gap-0.5">
            <h1
              class="text-lg sm:text-xl md:text-2xl font-black tracking-tight text-white leading-none">
              {{ currentAsset.name }}</h1>
            <span
              class="text-[10px] sm:text-xs font-bold text-zinc-400 bg-zinc-900/80 px-2 py-0.5 rounded-lg border border-zinc-800 w-fit">{{
                currentAsset.symbol
              }}</span>
          </div>
        </div>
        <div class="flex flex-col items-end gap-1">
          <div
            class="flex items-center gap-2 bg-zinc-900/90 border-2 border-yellow-500/20 px-3 py-1.5 rounded-xl shadow-lg shadow-yellow-900/10">
            <svg viewBox="0 0 24 24" fill="currentColor"
                 class="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-yellow-400">
              <path fill-rule="evenodd"
                    d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z"
                    clip-rule="evenodd"/>
            </svg>
            <span class="font-mono text-xl sm:text-2xl md:text-3xl font-bold text-yellow-100">{{
                score
              }}</span>
          </div>
          <div v-if="streak > 1"
               class="text-[10px] sm:text-xs font-bold text-orange-400 animate-pulse flex items-center gap-1">
            <span>🔥</span> x{{ streak }}
          </div>
        </div>
      </div>
      <div class="flex gap-2 bg-zinc-900/50 p-1.5 rounded-2xl border border-zinc-800">
        <button @click="setGameMode('candle')"
                ...
                :disabled="gameState !== 'playing'">1 Свеча
        </button>
        <button @click="setGameMode('trend')"
                ...
                :disabled="gameState !== 'playing'">Тренд
        </button>
      </div>
    </div>
    <div
      class="flex-1 w-full z-10 relative mt-2 flex flex-col min-h-0 bg-[#131722] border-y border-zinc-800">
      ...
      <transition
        ...
        <div v-if="showResultModal"
          ...
          <div
            class="flex flex-col items-center justify-center p-4 sm:p-6 rounded-[30px] border-4 shadow-2xl bg-zinc-900 min-w-[200px] sm:min-w-[250px] md:min-w-[300px]"
            :class="gameResult === 'win' ? 'border-green-500 text-green-400' : 'border-rose-500 text-rose-400'">
            <div class="text-5xl sm:text-6xl md:text-7xl mb-2 filter drop-shadow-lg">
              {{ gameResult === 'win' ? '🎉' : '💀' }}
            </div>
            <div
              class="text-xl sm:text-2xl md:text-3xl font-black uppercase tracking-widest text-white">
              {{ gameResult === 'win' ? 'ВЕРНО!' : 'МИМО!' }}
            </div>
            <div
              class="text-[10px] sm:text-xs font-bold mt-1 px-3 py-1 rounded-full bg-white/10 text-white">
    ...
    <div class="px-4 pt-4 z-20 shrink-0 h-24 sm:h-28 md:h-32 bg-zinc-950 flex items-start">
      ...
      <div v-if="gameState === 'playing'"
           class="grid grid-cols-2 gap-4 h-16 sm:h-20 md:h-24 w-full">
        <button @click="makeGuess('long')"
          ...
          <svg class="w-6 h-6 sm:w-8 sm:h-8 mb-1 group-hover:-translate-y-1 transition-transform"
               fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                  d="M5 10l7-7m0 0l7 7m-7-7v18"/>
          </svg>
          <span
            class="text-lg sm:text-xl md:text-2xl font-black tracking-wider group-hover:text-emerald-400">ВВЕРХ</span>
        </button>
        <button @click="makeGuess('short')"
          ...
          <span
            class="text-lg sm:text-xl md:text-2xl font-black tracking-wider group-hover:text-rose-400">ВНИЗ</span>
        </button>
      </div>

      <!-- ✅ ЛИМИТ ИСЧЕРПАН -->
      <div v-else-if="gameState === 'limitReached'"
           class="h-16 sm:h-20 md:h-24 w-full flex flex-col items-center justify-center text-center p-4">
      ...
      <!-- ✅ КНОПКА ДАЛЬШЕ -->
      <div v-else-if="gameState === 'result' && !showResultModal"
           class="h-16 sm:h-20 md:h-24 w-full">
      ...
      <div v-else class="h-16 sm:h-20 md:h-24 w-full flex items-center justify-center">
        <div
          class="w-8 h-8 border-4 border-zinc-600 border-t-yellow-400 rounded-full animate-spin"></div>
      </div>
    </div>

    <div
      class="fixed bottom-0 left-0 w-full bg-zinc-900/90 backdrop-blur-xl border-t border-white/5 z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
      <div class="flex justify-around items-center h-[70px] sm:h-[80px] md:h-[90px] px-2">
        <button @click="$router.push('/')"
                class="flex flex-col items-center justify-center gap-1 w-16 sm:w-20 h-full group">
          <svg
            class="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-zinc-500 group-hover:text-white transition-colors"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button @click="$router.push('/test')"
                class="flex flex-col items-center justify-center gap-1 w-16 sm:w-20 h-full group">
          <svg
            class="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-zinc-500 group-hover:text-green-400 transition-colors"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <button
          class="flex flex-col items-center justify-center gap-1 w-16 sm:w-20 h-full relative">
          <div
            class="absolute top-2 w-10 h-10 sm:w-12 sm:h-12 bg-purple-500/20 rounded-full blur-lg"></div>
          <svg class="w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 text-purple-400 relative z-10" fill="none"
        ...
        <button @click="$router.push('/test')"
                class="flex flex-col items-center justify-center gap-1 w-16 sm:w-20 h-full group">
          <svg
            class="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-zinc-500 group-hover:text-yellow-400 transition-colors"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"/>
          </svg>
          <span
            class="text-[10px] sm:text-xs font-medium text-zinc-500 group-hover:text-yellow-400">Награды</span>
        </button>
        <button @click="$router.push('/test')"
                class="flex flex-col items-center justify-center gap-1 w-16 sm:w-20 h-full group">
          <svg
...
const apiRequest = async (url: string, options: RequestInit = {}) => {
  ...
  console.log('🔍 TOKEN:', token ? 'ОК' : 'ПУСТОЙ!');
  console.log('🔍 URL:', url);
