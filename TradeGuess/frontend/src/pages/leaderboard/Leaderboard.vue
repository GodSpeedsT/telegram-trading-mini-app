<template>
  <div class="h-screen w-full flex flex-col items-center justify-center p-4 pb-[130px] pt-10 relative bg-zinc-950 overflow-hidden">
      ...
      <div class="flex flex-col items-center shrink-0 w-full px-5 pt-6 sm:pt-10 gap-4 sm:gap-6">
        <div class="w-14 h-14 sm:w-16 sm:h-16 bg-zinc-800 rounded-full border-2 border-zinc-700 flex items-center justify-center shadow-2xl relative overflow-visible shrink-0 transition-transform hover:scale-110">
          <div class="relative flex items-center justify-center w-full h-full">
            <svg viewBox="0 0 24 24" fill="currentColor" class="absolute w-10 h-10 text-yellow-500 blur-xl opacity-40 animate-pulse" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-else class="flex flex-col gap-2 sm:gap-3">
          <div v-for="entry in leaderboard.entries" :key="entry.user.id" class="relative w-full border transition-all duration-300 rounded-[16px] sm:rounded-[24px] flex items-center justify-between px-4 py-2.5 sm:py-4 overflow-hidden shrink-0"
            ...
            <div class="flex items-center gap-3 sm:gap-5 flex-1 min-w-0 z-10">
              ...
              <div class="w-8 h-8 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center text-zinc-200 font-black border border-white/5 shrink-0 shadow-lg text-[10px] sm:text-xs uppercase overflow-hidden">
              ...
              <div class="flex flex-col min-w-0">
                ...
                <span class="text-zinc-500 text-[9px] sm:text-[10px] truncate font-bold opacity-60 uppercase tracking-widest">@{{ entry.user.username }}</span>
              </div>
            </div>
      ...
      <div class="w-full flex flex-col shrink-0 bg-zinc-900 border-t border-zinc-800 z-40 px-6 py-5 sm:px-10 sm:py-7 gap-3 sm:gap-4">
        ...
        <div v-if="leaderboard" class="flex items-center justify-center">
          <div class="px-4 py-1.5 bg-zinc-950/60 border border-zinc-800 rounded-full flex items-center gap-2.5 backdrop-blur-sm shadow-inner">
            ...
            <span class="text-[9px] text-zinc-600 font-black uppercase tracking-widest whitespace-nowrap">Updated: {{ new Date(leaderboard.calculatedAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) }}</span>
          </div>
        </div>
      </div>
    </div>
...
.bg-puzzle-pattern {
  ...
  background-image: url("data:image/svg+xml,%3Csvg width='200' height='200' viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cdefs%3E%3Cpath id='p' d='M-2 0 v35 a15 15 0 0 1 0 30 v70 a15 15 0 0 0 0 30 v37 M-2 100 v35 a15 15 0 0 0 0 30 v37 M98 0 v35 a15 15 0 0 0 0 30 v70 a15 15 0 0 1 0 30 v37 M198 0 v35 a15 15 0 0 1 0 30 v70 a15 15 0 0 0 0 30 v37 M0 -2 h35 a15 15 0 0 1 30 0 h70 a15 15 0 0 0 30 0 h37 M0 98 h35 a15 15 0 0 0 30 0 h70 a15 15 0 0 1 30 0 h37 M0 198 h35 a15 15 0 0 1 30 0 h70 a15 15 0 0 0 30 0 h37' stroke-linecap='square' stroke-linejoin='round' fill='none'/%3E%3C/defs%3E%3Cuse href='%23p' stroke='%23000000' stroke-width='4' opacity='0.6' transform='translate(1,1)'/%3E%3Cuse href='%23p' stroke='%23ffffff' stroke-width='1.5' opacity='0.05'/%3E%3C/svg%3E");
