      ...
      <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="puzzlePattern" x="0" y="0" width="200" height="200" patternUnits="userSpaceOnUse">
            <path id="p" d="M-2 0 v35 a15 15 0 0 1 0 30 v70 a15 15 0 0 0 0 30 v37 M-2 100 v35 a15 15 0 0 0 0 30 v37 M98 0 v35 a15 15 0 0 0 0 30 v70 a15 15 0 0 1 0 30 v37 M198 0 v35 a15 15 0 0 1 0 30 v70 a15 15 0 0 0 0 30 v37 M0 -2 h35 a15 15 0 0 1 30 0 h70 a15 15 0 0 0 30 0 h37 M0 98 h35 a15 15 0 0 0 30 0 h70 a15 15 0 1 1 30 0 h37 M0 198 h35 a15 15 0 0 1 30 0 h70 a15 15 0 0 0 30 0 h37"
    ...
    <div class="relative z-10 w-full max-w-[98%] sm:max-w-[650px] md:max-w-[950px] lg:max-w-[1200px] xl:max-w-[1500px] flex flex-col items-center bg-zinc-900 border border-zinc-800 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden transition-all duration-500 rounded-[32px] md:rounded-[56px] h-fit max-h-[80vh]">
      ...
      <div class="flex items-center justify-between shrink-0 w-full px-5 md:px-12 py-4 md:py-6 bg-zinc-900/50 border-b border-zinc-800/50 gap-4">
        <div class="flex items-center gap-3 md:gap-5">
          ...
          <div class="flex flex-col text-left leading-none gap-1">
            <h1 class="text-xl md:text-3xl font-black text-white uppercase tracking-tighter">{{ block.title }}</h1>
            <span class="text-[8px] md:text-xs font-bold text-zinc-500 uppercase tracking-widest">{{ block.subtitle }}</span>
          </div>
        </div>
        <div class="flex items-center gap-4 shrink-0">
          <div class="flex items-center gap-2 px-3 py-1.5 md:px-5 md:py-2 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl shadow-inner">
            <svg viewBox="0 0 24 24" fill="currentColor" class="w-3 h-3 md:w-5 md:h-5 text-yellow-400">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
            </svg>
            <span class="text-[10px] md:text-xl font-black text-yellow-100 font-mono leading-none">{{ totalStars }}</span>
          </div>
          <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 md:px-5 md:py-2 bg-zinc-950/80 border border-zinc-800 rounded-2xl shadow-inner">
      ...
      <div class="flex-1 w-full overflow-y-auto no-scrollbar px-4 md:px-12 lg:px-16 py-4 md:py-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-4 lg:gap-5">
          <div v-for="achievement in achievements" :key="achievement.id" class="relative border transition-all duration-300 flex items-center p-3 md:p-5 lg:p-6 gap-3 md:gap-5 overflow-hidden rounded-2xl md:rounded-[32px] shrink-0"
            ...
            <div class="absolute top-2 right-2 flex items-center gap-1 bg-black/40 px-1.5 py-0.5 rounded-lg border border-white/5 z-10 shadow-sm">
              <svg viewBox="0 0 24 24" fill="currentColor" class="w-2.5 h-2.5 md:w-3 md:h-3 text-yellow-500">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
              </svg>
              <span class="text-[8px] md:text-[10px] font-black text-yellow-400 font-mono">{{ achievement.id % 2 === 0 ? 50 : 100 }}</span>
            </div>
            ...
            <div class="flex flex-col min-w-0 flex-1 gap-1">
              ...
              <p class="text-[9px] md:text-xs text-zinc-500 font-medium leading-tight line-clamp-2 pr-6">
...
interface AchievementWithStars extends Achievement {
  stars?: number;
}
